...
export interface VegetationFieldProps {
  ...
  /** Max random tilt in radians on both horizontal axes, simulating a natural lean - grass/thin stems only. Default 0 (upright), matching every existing caller's current look exactly. */
...
export function VegetationField({
  ...
  }, [seed, count, areaWidth, areaDepth, minScale, maxScale, maxLeanRadians, preComputedPoints]);
      ...
      <Instances limit={Math.max(1, instances.length)} range={instances.length} castShadow={castShadow}>
        ...
        {instances.map((instance, index) => (
          <Instance
            ...
            rotation={[instance.leanX, instance.rotationY, instance.leanZ]}
